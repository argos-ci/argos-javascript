...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------- | ------------------------------------------- |
| `build get`, `build snapshots`, `test list`, `test get`, `test changes`, `change list`, `project get`, `project deployments`, `project domain get`, `media get`, `media delete`                                                                                                                             | Project token               | `--token` › `ARGOS_TOKEN`                   |
| `review *`, `comment *`, `test comment *`, `test subscribe` / `unsubscribe`, `build subscribe` / `unsubscribe`, `change ignore` / `unignore`, `account *`, `project update`, `project transfer`, `project contributor *`, `project domain set`, `automation *`, `analytics`, `create-project`, `media list` | Personal access token (PAT) | `--token` › `ARGOS_TOKEN` › `argos login`   |
...
- **Media** — `media upload <files...> [--slug <slug>] [--pr <n> --comment] [--visibility team|public] [--retention <days>]` · `media list [--search <q>] [--type image|video]` · `media get|delete <mediaId>`
