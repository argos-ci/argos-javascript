...
import { METADATA_EXTENSION, PNG_EXTENSION, YAML_EXTENSION } from "./util";
...
function parseAttachmentName(name: string): {
  ...
  const match = name.match(
    /^argos\/(screenshot|snapshot)(\/metadata)?___(.*)$/,
  );
  if (!match) {
    return null;
  }
...
export function getAttachmentFilename(attachment: Attachment): string {
  ...
  const extension = {
    ...
    snapshot: YAML_EXTENSION,
    "screenshot/metadata": `${PNG_EXTENSION}${METADATA_EXTENSION}`,
    "snapshot/metadata": `${YAML_EXTENSION}${METADATA_EXTENSION}`,
  }[type];
...
export function checkIsArgosSnapshot(
  ...
  return parsed.type === "snapshot" || parsed.type === "screenshot";
}
...
export function checkIsArgosMetadata(
  ...
  return (
    parsed.type === "snapshot/metadata" || parsed.type === "screenshot/metadata"
  );
}
