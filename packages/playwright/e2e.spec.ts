...
async function expectSnapshotToExists(
  ...
  type: "screenshot" | "aria-snapshot" = "screenshot",
) {
  ...
  const extension = { screenshot: "png", "aria-snapshot": "yaml" }[type];
...
test.describe("#argosAriaSnapshot", () => {
  test("takes a an aria snapshot of the page", async ({ page }) => {
    ...
    await expectSnapshotToExists("body-aria-snapshot", "aria-snapshot");
  });
  ...
  test("takes a an aria snapshot of the transparent section", async ({
    ...
    await expectSnapshotToExists("section-aria-snapshot", "aria-snapshot");
  });
});
